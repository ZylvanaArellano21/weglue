...
import { editPostCaption, removePostFromClub, editCommentContent } from "../contentActions";
...
describe("permanent-delete denial", () => {
  it("exposes no destructive delete/hide export for posts or comments", () => {
    const names = Object.keys(contentActions);
    expect(names).not.toContain("deletePost");
    expect(names).not.toContain("deleteComment");
    expect(names).not.toContain("hidePost");
    expect(names).not.toContain("hideComment");
    // Only the safe, canonical operations are exported.
    expect(names.sort()).toEqual(["editCommentContent", "editPostCaption", "removePostFromClub"]);
  });
});
