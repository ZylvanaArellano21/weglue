...
// The interest list for BOTH mobile interest surveys (onboarding + Edit
// Interests), read from the shared `interests` catalog so an admin change shows
// up with no app release. Falls back to the bundled canonical list only if the
...
export function useInterestCatalog(): InterestCatalogState {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const options = await fetchActiveInterests(supabase);
        if (cancelled) return;
        if (options.length === 0) {
          setState({
