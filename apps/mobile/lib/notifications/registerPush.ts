...
// token actually changed, the SIGNED-IN USER changed, or once every few hours
// as a self-heal in case the server-side record was invalidated by push-receipt
// reconciliation. The user check matters when a second account signs in on the
// same device: the Expo token is unchanged, but register_push_token must run so
// the server row moves to the new user (deactivate-on-logout is best-effort and
...
export async function registerPushTokenIfPermitted(): Promise<string | null> {
  try {
    ...
    // register_push_token is auth-scoped (writes the row for auth.uid()); with
    // no session there is nothing to register.
    ...
    if (
      ...
      Date.now() - lastRegisteredAt < REGISTER_REFRESH_MS
    ) {
      return token;
    }
