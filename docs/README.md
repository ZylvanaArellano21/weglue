...
- [`deleted-message-privacy.md`](product/deleted-message-privacy.md) — **authoritative v5 design** for secure deleted-message privacy (corrected client facts, attempt model, attachment mapping, orchestration, reconciliation worker, report evidence, poll + push privacy, OTA evidence). Design only, not implemented.
