...
-- This complements the 069 harness with cross-platform safety regressions:
...
BEGIN
  ...
  IF v_definition NOT LIKE '%BEFORE INSERT OR UPDATE%' THEN
    RAISE EXCEPTION '070: event_end_at trigger must run on every event update';
  END IF;
  ...
  IF has_function_privilege('authenticated', 'private.can_access_event(uuid,uuid)', 'EXECUTE')
    ...
    RAISE EXCEPTION '070: event access helper grants are not caller-bound';
  END IF;
  ...
  IF position('NOT (selected_id = ANY(v_existing_ids))' IN v_definition) = 0 THEN
    RAISE EXCEPTION '070: audience validation must distinguish existing recipients from additions';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_events_broadcast_audience_sync'
  ) THEN
    RAISE EXCEPTION '070: selected-audience changes must publish an opaque convergence signal';
  END IF;
END;
$$;
...
INSERT INTO auth.users (id, email, raw_app_meta_data) VALUES
  ('11000000-0000-0000-0000-000000000001', 'creator-070@example.test', '{}'::jsonb),
  ('11000000-0000-0000-0000-000000000002', 'former-070@example.test', '{}'::jsonb),
...
BEGIN
  ...
  IF v_cdt <> '2099-01-02 20:00:00+00'::timestamptz THEN
    RAISE EXCEPTION '070: expected America/Chicago CST conversion, got %', v_cdt;
  END IF;
  ...
  IF v_cdt <> '2099-07-01 19:00:00+00'::timestamptz THEN
    RAISE EXCEPTION '070: expected America/Chicago CDT conversion, got %', v_cdt;
  END IF;
  ...
  IF v_cst <> v_cdt THEN
    RAISE EXCEPTION '070: direct event_end_at override was accepted';
  END IF;
END;
$$;
...
BEGIN
  IF NOT private.can_access_event(
    ...
    RAISE EXCEPTION '070: unchanged former recipient lost historical access';
  END IF;
  ...
  BEGIN
    UPDATE public.events
     ...
     WHERE id = '31000000-0000-0000-0000-000000000001';
    RAISE EXCEPTION '070: stale former recipient was re-added';
...
BEGIN
  BEGIN
    ...
    RAISE EXCEPTION '070: authenticated caller invoked arbitrary-viewer predicate';
  EXCEPTION WHEN insufficient_privilege THEN
    NULL;
  END;

  IF EXISTS (SELECT 1 FROM public.events WHERE id = '31000000-0000-0000-0000-000000000002') THEN
    RAISE EXCEPTION '070: outsider read selected event payload';
  END IF;
END;
$$;
...
SELECT '070 cross-platform event permission parity harness passed' AS result;
